...
async def porn(ctx, *args):
    ...
    if answer:
        if 'webm' in answer:
            ...
            embed = disnake.Embed(title=f'Rule34: {arg}', color=ctx.author.color)
...
async def rcoin(ctx):
	...
	if side == 50 or side > 50:
		...
		embed = disnake.Embed(title=f'NSFW Coinflip: Heads', color=ctx.author.color)
		...
		if 'mp4' in url:
			...
			return await ctx.channel.send(url)
		embed = disnake.Embed(title=f'NSFW Coinflip: Tails', color=ctx.author.color)
		embed.set_author(name=f'{ctx.author.display_name} - NSFW Coinflip',icon_url=f'{ctx.author.avatar.url}')
		embed.set_image(url=url)
		print(url)
		embed.set_footer(text='Pornbot 2.0 - Made by jess#3347')
...
async def fcoin(ctx):
	...
	if side == 50 or side > 50:
		...
		if 'mp4' in url:
			...
			return await ctx.channel.send(url)
		embed = disnake.Embed(title=f'Furry Coinflip: Heads', color=ctx.author.color)
		...
		if 'mp4' in url:
			...
			return await ctx.channel.send(url)
		embed = disnake.Embed(title=f'Furry Coinflip: Tails', color=ctx.author.color)
		embed.set_author(name=f'{ctx.author.display_name} - Furry Coinflip',icon_url=f'{ctx.author.avatar.url}')
		embed.set_image(url=url)
		embed.set_footer(text='Pornbot 2.0 - Made by jess#3347')
		await ctx.channel.send(embed=embed)
...
# ================================================================================================================
@client.command()
async def help(ctx: disnake.Message):
